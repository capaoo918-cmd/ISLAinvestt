...
async function updateSchemaAndData() {
  try {
    ...
    // 1. Alterar tabla proyectos para añadir nuevos campos técnicos
    await pool.query(`
    ...
    console.log("Esquema actualizado. Poblando datos técnicos para las propiedades actuales...");

    // 2. Definir una lista de amenidades estándar
    ...
    // 3. Obtener proyectos actuales
    const res = await pool.query("SELECT id_proyecto, titulo FROM proyectos");
    
    for (const row of res.rows) {
      // Asignar datos realistas basados en el título o aleatorios para completar
      ...
      // Seleccionar 5 amenidades aleatorias
    ...
    console.log("Migración de datos técnicos completada.");
    process.exit(0);
